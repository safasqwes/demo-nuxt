  ...
  <div class="min-h-screen bg-gradient-to-br from-green-50 to-gray-50 py-12">
    <div class="max-w-2xl mx-auto px-5 text-center">
      ...
      <div v-if="orderDetails" class="bg-white rounded-2xl shadow-xl p-8 mb-8 text-left">
        <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ $t('paymentSuccess.orderDetails.title') }}</h2>
        <div class="space-y-4">
          <div class="flex justify-between">
            <span class="text-gray-600">{{ $t('paymentSuccess.orderDetails.orderId') }}</span>
            <span class="font-mono text-sm">{{ orderDetails.orderId }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">{{ $t('paymentSuccess.orderDetails.amount') }}</span>
            <span class="font-semibold">{{ orderDetails.tokenAmount }} {{ orderDetails.currency }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">{{ $t('paymentSuccess.orderDetails.description') }}</span>
            <span>{{ orderDetails.description }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">{{ $t('paymentSuccess.orderDetails.status') }}</span>
          ...
          <div v-if="orderDetails.transactionHash" class="flex justify-between">
            <span class="text-gray-600">{{ $t('paymentSuccess.orderDetails.transaction') }}</span>
      ...
      <div class="bg-blue-50 rounded-2xl p-6 mb-8">
        <h3 class="text-lg font-semibold text-gray-900 mb-3">{{ $t('paymentSuccess.nextSteps.title') }}</h3>
        <ul class="text-left space-y-2 text-gray-700">
          <li class="flex items-start">
            <svg class="w-5 h-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
            </svg>
            {{ $t('paymentSuccess.nextSteps.step1') }}
          </li>
          <li class="flex items-start">
            <svg class="w-5 h-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
            </svg>
            {{ $t('paymentSuccess.nextSteps.step2') }}
          </li>
          <li class="flex items-start">
            <svg class="w-5 h-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
            </svg>
            {{ $t('paymentSuccess.nextSteps.step3') }}
          </li>
        </ul>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center">
        <NuxtLink to="/" 
          ...
          {{ $t('paymentSuccess.actions.continueReading') }}
        </NuxtLink>
        <NuxtLink to="/pricing" 
          ...
          {{ $t('paymentSuccess.actions.browseMore') }}
        </NuxtLink>
      </div>
    </div>
  </div>
...
const { t } = useI18n()
useHead({
  title: t('paymentSuccess.seo.title'),
  meta: [
    { name: 'description', content: t('paymentSuccess.seo.description') }
  ]
})
